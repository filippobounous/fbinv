...
def update_all_price_histories(intraday: bool = False, local_only: bool = False) -> Dict[str, Dict[str, bool]]:
    di = {}
    for ds in tqdm(all_datasource, desc="Updating all data sources"):
        ds_instance: "BaseDataSource" = ds()
        di[ds_instance.name] = ds_instance.update_all_price_histories(intraday=intraday, local_only=local_only)
    return di
...
def update_all(intraday: bool = False, local_only: bool = False) -> Tuple[Dict[str, bool], Dict[str, Dict[str, bool]]]:
    ...
    di_timeseries = update_all_price_histories(intraday=intraday, local_only=local_only)
