...
from ..utils.consts import DEFAULT_RV_MODEL, DEFAULT_RV_WIN_SIZE, HL, OHLC, TRADING_DAYS
...
class RealisedVolatilityCalculator(BaseAnalytics):
    ...
    @staticmethod
    def registry() -> (
        dict[str, dict[str, Callable[[pd.DataFrame, int], pd.Series] | list[str]]]
    ):
    ...
    def _gk_yang_zhang(self, df: pd.DataFrame, rv_win_size: int) -> pd.Series:
        ...
        gk = self._garman_klass(
            df=df, rv_win_size=rv_win_size, _return_initial_term=True
        )
    ...
    def _yang_zhang(self, df: pd.DataFrame, rv_win_size: int) -> pd.Series:
        ...
        rs2 = (
            self._rogers_satchell(df, rv_win_size) ** 2
        )  # already annualised, hence squared here
